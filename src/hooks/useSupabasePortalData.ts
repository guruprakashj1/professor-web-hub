...
import { SupabasePortalStorageService } from '@/utils/supabasePortalStorage';
import { PortalData } from '@/types/portalData';

export const useSupabasePortalData = () => {
  const [data, setData] = useState<PortalData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  const storage = SupabasePortalStorageService.getInstance();
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setData(portalData);
      setError(null);
      console.log('Data loaded from Supabase:', portalData);
    } catch (err) {
